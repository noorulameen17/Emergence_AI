...
const Starfield = ({
  ...
  const [state, setState] = useState({
    init: true,
    canvas: true,
    start: false, // prevent a second init from the effect below
    stop: false,
    destroy: false,
    reset: false,
    uid: generateUUID(),
  ...
  const resize = () => {
    ...
    measureViewport();
    if (!sd.current.ctx || !sd.current.ctx.canvas) return; // guard: context not ready or destroyed
  ...
  const draw = () => {
    ...
    if (!ctx) return; // guard if context missing
  ...
  useEffect(() => {
    ...
    if (state.start) {
      // init() intentionally not auto-run here to avoid double initialization
